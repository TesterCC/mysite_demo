...
class SysSendEmail(models.Model):
    ...
    email = models.EmailField(max_length=50, verbose_name=u'邮箱', blank=True, null=True)
    ...
    class Meta:
        ...
        verbose_name = u'发送邮件管理'
        verbose_name_plural = verbose_name

        index_together = (('event_id', 'email'),)


class SysSendSms(models.Model):
    ops_type = ((0, u'默认发送'), (1, u'主办方审核'), (2, u'下单成功'), (3, u'编辑通过会议审核'))
    send_status = ((0, u'待发送'), (1, u'发送成功'), (2, u'发送失败'))
    ...
    send_flag = models.BooleanField(default=0, verbose_name='是否发送', help_text='是否发送')  # 针对非即时发送，判断是否需要发送消息
    status = models.PositiveSmallIntegerField(verbose_name=u'投递状态', default=0, choices=send_status)
    tel = models.CharField(verbose_name=u'联系电话', max_length=11, null=True, blank=True)
    ...
    class Meta:
        ...
        verbose_name = u'发送短信管理'
        verbose_name_plural = verbose_name

        index_together = (('event_id', 'tel'),)
